...
export type AiTaskPriority = 1 | 2 | 3 | 4;
...
export interface AiTask {
  ...
  attachments?: AiTaskAttachment[] | null;
  createdAt: string;
  updatedAt: string;
}
...
const STATUS_OPTIONS: Array<{ value: AiTaskStatus; label: string }> = [
...
const PRIORITY_OPTIONS: Array<{ value: AiTaskPriority; label: string }> = [
...
@Component({
  ...
  <div class="max-w-6xl mx-auto">
    ...
    <div class="flex items-start justify-between mb-6">
      <div>
        ...
        <h1 class="mc-heading text-3xl font-bold mt-1 flex items-center gap-2">
          <i class="fa-solid fa-gear text-brand"></i> AI Tasks
        </h1>
        <p class="text-fg-muted mt-1 text-sm">Track tasks the AI Code worker is queued for, working on, and has completed.</p>
      </div>
    ...
    <div class="flex flex-wrap items-center gap-4 mb-6 p-3 mc-card">
      ...
      <label class="flex items-center gap-2 cursor-pointer select-none" title="Enable/disable the AI Code worker from picking up tasks">
        <div class="relative">
    ...
    <div class="flex flex-wrap items-center gap-3 mb-4 text-sm">
      ...
      <div class="ml-auto text-xs text-fg-subtle">Worker polls <code>/v1/ai-tasks/next</code></div>
    </div>
        ...
        <div class="px-6 py-4 overflow-y-auto flex-1">
          <form [formGroup]="taskForm" class="space-y-4">
            ...
            <div>
              ...
              <textarea class="mc-input min-h-32" formControlName="description" placeholder="Full brief for AI Code…"></textarea>
            </div>
            <div>
...
export class AiPage implements OnInit, OnDestroy {
  ...
  protected readonly allTasks = signal<AiTask[]>([]);
  ...
  protected readonly editing = signal<Partial<AiTask> | null>(null);
  ...
  protected readonly templates = signal<AiPromptTemplate[]>([]);
  ...
  protected readonly filteredTasks = computed(() => {
    ...
    let working = tasks;
    const isTerminal = (s: AiTaskStatus) =>
    ...
    const sorter = (a: AiTask, b: AiTask) => {
      switch (order) {
  ...
  private connectSSE() {
    this.zone.runOutsideAngular(() => {
      ...
      const es = new EventSource(`${this.api.base}/ai-tasks/events?token=${encodeURIComponent(token)}`);
  ...
  protected countOf(status: AiTaskStatus): number {
    return this.allTasks().filter((t) => t.status === status).length;
  }

  loadTasks() {
    this.loading.set(true);
    this.api.get<AiTask[]>('/ai-tasks').subscribe({
  ...
  loadSettings() {
    this.api.get<{ workerActive: boolean; scannerActive: boolean; deployNext: boolean }>('/ai-tasks/settings').subscribe({
      next: (s) => {
  ...
  toggleSetting(key: 'workerActive' | 'scannerActive' | 'deployNext', enabled: boolean) {
    ...
    this.api.patch<{ workerActive: boolean; scannerActive: boolean; deployNext: boolean }>('/ai-tasks/settings', { [key]: enabled }).subscribe({
      next: (s) => {
        this.settingsWorkerActive.set(s.workerActive);
        this.settingsScannerActive.set(s.scannerActive);
        this.settingsDeployNext.set(s.deployNext);
  ...
  submitDeferredReply() {
    ...
    this.api.patch<AiTask>(`/ai-tasks/${t.id}`, { status: 'PENDING', notes }).subscribe({
      next: () => {
  ...
  saveTask() {
    ...
    if (t.id) {
      this.api.patch<AiTask>(`/ai-tasks/${t.id}`, body).subscribe({ next: done, error: fail });
    } else {
      this.api.post<AiTask>(`/ai-tasks`, body).subscribe({ next: done, error: fail });
    }
  }
  ...
  deleteTask(t: AiTask) {
    this.notify.confirm(
      ...
      if (!ok) return;
      this.api.delete(`/ai-tasks/${t.id}`).subscribe({
  ...
  removeAttachment(a: AiTaskAttachment) {
    const t = this.editing();
    if (!t?.id) return;
    // The filename in storage is "ai-tasks-<id>/<hash>.ext"; we use the
    // basename for the URL segment.
    const basename = a.filename.split('/').pop()!;
    this.api.delete(`/ai-tasks/${t.id}/attachments/${basename}`).subscribe({
      next: () => {
  ...
  openTemplates() {
    ...
    this.api.get<AiPromptTemplate[]>('/ai-prompt-templates').subscribe({
  ...
  createTemplate() {
    ...
    this.api.post<AiPromptTemplate>('/ai-prompt-templates', {
  ...
  deleteTemplate(t: AiPromptTemplate) {
    this.notify.confirm(
      `Delete template?`,
      ...
      if (!ok) return;
      this.api.delete(`/ai-prompt-templates/${t.id}`).subscribe({
        next: () => {
  ...
  useTemplate(t: AiPromptTemplate) {
    this.closeTemplates();
    this.openNewTask();
  ...
  // Style helpers ------------------------------------------------------------
  protected statusIcon(s: AiTaskStatus): string {
    return {
  ...
  protected statusLabel(s: AiTaskStatus): string {
    return STATUS_OPTIONS.find((x) => x.value === s)?.label ?? s;
  }
  protected priorityLabel(p: AiTaskPriority): string {
    return PRIORITY_OPTIONS.find((x) => x.value === p)?.label ?? `Priority ${p}`;
  }
  protected statusBadgeClass(s: AiTaskStatus): string {
    return {
  ...
  protected priorityBadgeClass(p: AiTaskPriority): string {
    return {
