...
- TO_BE_DEPLOYED — done locally but not yet deployed (rare; usually skipped).
- COMPLETED      — done and (where applicable) deployed.
- FAILED         — terminal. The worker tried and could not finish. Operator reviews and (optionally) re-queues by creating a new task. findNext() does NOT pick FAILED back up — no infinite retry loops.
...
2. **Batch sniff (parallelism)**. After claiming task A, do `GET /v1/claude-tasks` and filter to PENDING tasks. Try to identify up to **3 additional** tasks (4 total including A) that are INDEPENDENT of each other AND of A. Independent means:
   ...
   - No shared dependencies (e.g. both editing `prisma/schema.prisma` is NOT independent; both adding the same shared util is NOT independent).
...
4. **Execute**:
   ...
   **(b) Parallel batch (size 2-4)** -- spawn one `Agent` per task in a **single message with multiple Agent tool uses** so they run concurrently. Each agent's prompt MUST be self-contained and MUST include:
   ...
   - **Status-discipline clause** (verbatim in every agent prompt):
     > "You OWN this task's queue status. On success PATCH /v1/claude-tasks/{id} to `{status:'COMPLETED', notes:'Commit <sha>. <one-line proof>.'}`. On failure or blocker PATCH to `{status:'FAILED', notes:'Blocked: <why>.'}` — do NOT flip back to PENDING, that would re-queue an unsolvable task. Never leave the task IN_PROGRESS. Never touch any task ID other than your own."
...
7. **On failure/ambiguity** for solo work -> PATCH the task to FAILED with notes, then pick the next task this iteration. FAILED is terminal — findNext() will not return it again, so the worker won't loop on an unsolvable task. Operator triages FAILED rows on /app/claude.
...
3. Read `rawOutput` — it is a full prompt describing the JSON schema for the site spec.
