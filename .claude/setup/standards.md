# MADCreate â€” Application Standards
...
> Angular 19 app styled with **Tailwind + custom `mc-*` primitives** â€”
> no Fuse, no Angular Material. The API is **.NET Core 11 + Entity Framework Core 5 +
> MSSQL + Redis** (BullMQ). When this file and inline code comments
...
| Forms    | Reactive forms via `FormBuilder` â€” never template-driven for non-trivial forms                        |
| API      | .NET Core 11, modular per feature (`apps/api/src/modules/*`)                                             |
| ORM      | Entity Framework Core 5 (`Entity Framework Core/schema.Entity Framework Core`). MSSQL on `mssql.madcreate.madleads.ai`.                            |
| Queue    | BullMQ on Redis 7 (`@.NET Core/bullmq`, `ioredis`). Used for AI generation jobs.                         |
| Auth     | JWT bearer via interceptor + `X-Worker-Token` constant-time bypass for the autonomous worker          |
| Deploy   | `pwsh deploy.ps1` â€” Posh-SSH SFTP + remote `Entity Framework Core db push` + PM2 reload of `mc-api` on DreamHost CloudCompute |
...
them at runtime per tenant â€” never hard-code a hex in a component.
...
| Dark mode          | `darkMode: 'class'` â€” `.dark` selector on `<html>`. Default = dark.                            |
...
Tailwind defaults â€” **not** overridden in `tailwind.config.js`. Use
...
| Property        | Value                                                                                              |
|-----------------|----------------------------------------------------------------------------------------------------|
| Main shell      | `DashboardLayoutComponent` at `apps/web/src/app/layouts/dashboard/` â€” provides sidebar + topbar slot |
| Auth shell      | Standalone routes mount pages at top level â€” no auth-only layout component                         |
| Content padding | `px-4 md:px-6 lg:px-8 py-6 md:py-8` â€” set inside each feature page                                 |
...
`styles.scss` â€” **never** ad-hoc `bg-gray-*` / `border-gray-*`.

| Variant          | Class            | Usage                                          |
|------------------|------------------|------------------------------------------------|
| Resting card     | `mc-card`        | Most surfaces â€” list rows, summary tiles       |
...
- **Disabled buttons still need a tooltip** explaining *why* they're disabled
  (e.g. `title="Save â€” fix validation errors first"`).
- **Min touch target 44Ã—44 px** (WCAG 2.5.5). Use
...
- **Tooltip implementation:** today, the native HTML `title` attribute.
  ...
  theme), add a custom `appTooltip` directive â€” don't pull in Material
...
| Surface                                                 | Reason                                                                                          |
|---------------------------------------------------------|-------------------------------------------------------------------------------------------------|
| Marketing pages (`/`, `/pricing`, `/about`, `/contact`) | First-time visitors â€” affordance > density. CTAs like "Start free trial" are full text.         |
| Onboarding wizard (6 steps)                             | Users have never seen the app â€” "Next â†’", "Back", "Generate site" stay as text.                  |
| Confirmation dialogs (notification confirm/cancel)      | The two buttons are the entire decision â€” use text ("Delete", "Cancel") so a misclick isn't catastrophic. |
...
| Label          | `<label class="mc-label">â€¦</label>` immediately preceding the input      |
| Textarea       | `<textarea class="mc-input min-h-32">`                                   |
| Select         | `<select class="mc-input">â€¦</select>`                                    |
| Reactive forms | `FormBuilder.nonNullable.group({...})` â€” never template-driven for non-trivial forms |
...
@if (open()) {
  <div class="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm grid place-items-center p-4">
    <div class="mc-card w-full max-w-xl max-h-[90vh] flex flex-col">
      ...
      <!-- content -->
      <div class="px-6 py-4 overflow-y-auto flex-1">â€¦</div>
      ...
      <div class="px-6 py-3 border-t border-white/5 flex items-center justify-end gap-2">
        ...
        <button class="mc-btn-primary !px-3 !py-2" (click)="save()"
                ...
                [title]="saving() ? 'Savingâ€¦' : (form.invalid ? 'Save â€” fix validation errors first' : 'Save')">
...
Modals only close via explicit close/cancel/submit buttons â€” never on backdrop click. Width:
...
The confirm dialog's two buttons are a Button Standard carve-out â€” they
...
- **Card list** â€” `<div class="space-y-2">` of `mc-card-hover` rows.
- **Grid** â€” `grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3` of `mc-card`.
- **HTML table** â€” `<table class="w-full">` with custom Tailwind row styles, ONLY where columnar data is genuinely tabular.

Always provide:
- Empty state (`mc-card p-12 text-center`) when zero rows.
- Skeleton loaders (block `bg-white/10 animate-pulse`) while initial fetch is in flight â€” never blank-while-loading.
...
| Real data only   | No hard-coded mock arrays in components â€” every value from a live API call.                |

## Angular Code Standard

| Rule                                | Why                                                                |
|-------------------------------------|--------------------------------------------------------------------|
| `standalone: true` everywhere       | Project convention â€” never `NgModule`-declared.                    |
| Control flow                        | `@if`, `@for`, `@switch` â€” NEVER `*ngIf`, `*ngFor`, `*ngSwitch`.    |
...
| File paths                          | Always `C:/Code/madcreate/...` in this repo â€” never other user home directories. |

## .NET Core / Entity Framework Core Standard

| Rule                          | Why                                                                                          |
|-------------------------------|----------------------------------------------------------------------------------------------|
| Schema source of truth        | `Entity Framework Core/schema.Entity Framework Core`. Run `npx Entity Framework Core generate` after edits; `Entity Framework Core db push` runs at deploy. |
| Enums                         | Add to `schema.Entity Framework Core`, then mirror in `class-validator` `@IsIn(['â€¦'] as const)` in the DTO. |
| Route ordering                | Literal-segment routes (`@Get('next')`, `@Post('import-bulk')`) MUST come before `:id` params â€” `ParseIntPipe` 400s otherwise. |
| DTOs                          | Every property needs a `class-validator` decorator. `ValidationPipe` runs with `whitelist: true, forbidNonWhitelisted: true`. |
| Partial updates               | Service `update()` must check `if (dto.field !== undefined)` â€” the worker PATCHes partial bodies. |
| Transactions                  | Long-running ops use `$transaction(..., { timeout: 30_000 })` (DreamHost MSSQL has a tight idle timeout). |
...
| Step       | Command                                                                                                |
|------------|--------------------------------------------------------------------------------------------------------|
| Redis      | `redis-server --daemonize yes` â€” must run before API (BullMQ).                                         |
| API        | `npm --workspace @madcreate/api run start:dev` â€” `.NET Core start --watch`, port 3000 in dev.               |
| Frontend   | `cd apps/web && npm start` â€” `ng serve --host 0.0.0.0 --port 4200`.                                    |
| Build FE   | `npm --workspace @madcreate/web run build` (production via `angular.json` config).                     |
| Build API  | `npm --workspace @madcreate/api run build` (`.NET Core build`).                                             |
| Entity Framework Core     | `npx Entity Framework Core generate` after schema edits.                                                              |
| Deploy     | `pwsh deploy.ps1` from repo root â€” only when explicitly requested. Builds, atomic-swaps FE + API on the VPS, runs `Entity Framework Core db push`, reloads PM2, verifies `/v1/health`. |
...
| Property             | Value                                                                                                            |
|----------------------|------------------------------------------------------------------------------------------------------------------|
| Queue list endpoint  | `GET /v1/claude-tasks` (bucketed: active â†’ to-deploy â†’ terminal)                                                 |
| Worker poll endpoint | `GET /v1/claude-tasks/next` â€” 200 `{ ok, data: { task } }` or 204 empty                                          |
| Bulk import          | `POST /v1/claude-tasks/import-bulk` â€” dedupes by trim+lowercase title against active queue (PENDING + IN_PROGRESS) |
| Auth                 | All endpoints behind `JwtAuthGuard` + `X-Worker-Token` bypass                                                    |
| Statuses             | `PENDING`, `IN_PROGRESS`, `TO_BE_DEPLOYED`, `COMPLETED`, `CANCELLED`, **`FAILED`** (terminal, no retry), **`DEFERRED`** (operator-input needed â€” flip back to PENDING when ready) |
| Priority             | `1=Critical, 2=High, 3=Normal (default), 4=Low`                                                                   |
| Worker cron          | Windows Task Scheduler `MADCreateWorker` â€” every 1 minute, adaptive stride 1m â†’ 5m â†’ 10m â†’ 30m â†’ 1h via empty-streak counter, reset on hit. Prompt: `.claude/worker/worker-prompt.md`. |
| Scanner cron         | Windows Task Scheduler `MADCreateScanner` â€” every 1 hour. Read-only scan; POSTs new findings via import-bulk. Prompt: `.claude/scanner/scanner-prompt.md`. |
...
| Parallel agents      | Worker may spawn 2â€“4 `Agent` tool uses concurrently when it identifies independent tasks. Site-generation tasks always run solo. |

---

*Update this file whenever a new pattern lands. Stale standards docs are
worse than missing ones â€” when adding a section, also update the
