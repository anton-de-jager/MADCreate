...
A single PowerShell script (`deploy/deploy.ps1`) handles the full publish: builds the Angular app with the production API URL baked in, builds .NET Core, ships a single tarball over SFTP, runs `Entity Framework Core db push` against the production MSSQL, then runs `pm2 reload mc-api` (or starts it on first run) over SSH. The Apache vhost in front of `madcreateapi.madprospects.com` reverse-proxies all requests to the PM2 process via `mod_proxy`. **Passenger is intentionally not used â€” it's broken on this hosting.**

Credentials and per-environment values live in **`.deploy/.env.deploy`** (gitignored) . Edit them there â€” never in `deploy/deploy.ps1`.
...
```
            ...
            â”‚  static SPA bundle   â”‚    â”‚ .htaccess  [P]     â”‚
            â”‚ /home/<user>/        â”‚    â”‚ reverse-proxy to   â”‚
            â”‚ madcreate.madleadsâ€¦/ â”‚    â”‚ 127.0.0.1:<PORT>   â”‚
...
```bash
# Node + PM2 are required. Versions matter â€” Node 20+ for .NET Core 11.
node -v                              # must be 20+
npm -v
pm2 -v || sudo npm i -g pm2

# One-time: register the systemd unit so PM2 survives reboot.
# This prints a `sudo â€¦` command â€” copy/paste it as-is to enable the service.
pm2 startup
...
In **Domains â†’ Manage Domains â†’ Add Hosting**:
...
Default port for this stack is **3005** (mh-api occupies 3002 on the same box). If you pick a different port, update `API_PORT` in `.deploy/.env.deploy` **and** the vhost config â€” they must match.

### 5. TLS certificates

In **Domains â†’ Manage Domains â†’ Secure Hosting**, add a free **Let's Encrypt** certificate for *both* subdomains.

### 6. SFTP/SSH user with shell access

The deploy script needs SSH (not just SFTP) to extract the tarball, run `Entity Framework Core db push`, and run PM2. The same user can own both subdomains' web roots. **Shell access must be enabled** on the user in the panel.

### 7. MSSQL

You can use DreamHost's managed MSSQL or MSSQL running on the CloudCompute box itself. Wherever it runs:

- Database: `madcreate`
- User with full access to that database
- Hostname/port reachable from the CloudCompute instance

`Entity Framework Core db push` runs from the API host after every deploy (unless you pass `-SkipDbPush`) so the schema stays in sync. No manual migration files needed in dev mode.
...
API_REMOTE_PATH=/home/<user>/madcreate-api              # PM2 app dir â€” OUTSIDE web root
...
DB_HOST=mssql.madcreate.madprospects.com
DB_PORT=1433
...
# Optional â€” defaults to FRONTEND_PUBLIC_URL. Comma-separated for multiple origins.
...
What `-Seed` does after upload: SSHes to the API host and runs `npx tsx database/Entity Framework Core/seed.ts`, which populates:
...
The seed is idempotent â€” re-running won't duplicate.
...
```powershell
.\deploy\deploy.ps1                    # build both, upload both, Entity Framework Core db push, pm2 reload mc-api
...
.\deploy\deploy.ps1 -Seed              # re-run the seed (idempotent â€” won't duplicate)
```
...
```
index.html
.htaccess          â† SPA rewrite: /app/xxx etc. â†’ index.html
main-*.js
polyfills-*.js
styles-*.css
assets/
...
```
app.js                â† PM2 entry: dotenv + require('./dist/main')
package.json          â† Modified: @madcreate/shared as file: ref, Entity Framework Core in prod deps
package-lock.json
.env                  â† Prod DB creds + JWT secrets (auto-generated, persisted under .deploy/)
dist/                 â† Compiled .NET Core
database/Entity Framework Core/               â† schema.Entity Framework Core + seed.ts (used by Entity Framework Core generate / db push / seed)
shared-pkg/           â† Bundled @madcreate/shared (dist + package.json)
scripts/              â† Operator scripts (e.g. reset-super-admin.js)
node_modules/         â† Bundled into the tarball at build time; Entity Framework Core client regenerated on the server
storage/uploads/      â† Tenant media uploads (persisted across deploys)
```

**Apache web root** (`<API_WEB_ROOT>`, e.g. `/home/<user>/madcreateapi.madprospects.com/`):
```
.htaccess             â† Reverse-proxy to 127.0.0.1:<API_PORT> via [P]
```
...
4. **Rewrites the staged `package.json`**: replaces `"@madcreate/shared": "*"` with `"@madcreate/shared": "file:./shared-pkg"`, and moves `Entity Framework Core` into prod deps so the CLI is available on the server for `db push`.
...
7. On the server, runs `npx Entity Framework Core generate` so the Entity Framework Core client uses the Linux-native engine (the version cached in your local node_modules won't run on Ubuntu).

This means the server never has to know about pnpm workspaces or our monorepo layout â€” it sees a single self-contained Node app.
...
```
If that returns 200, the issue is Apache config â€” confirm `mod_proxy_http` is enabled and that the `.htaccess` actually landed at `<API_WEB_ROOT>/.htaccess`.

**Entity Framework Core fails at startup with "Cannot find module @Entity Framework Core/client"**
The `Entity Framework Core generate` step didn't run, or it generated against the wrong platform. Re-run the deploy (the script regenerates on every deploy). If it persists, SSH in and:
```
cd /home/madcreate-api
npx Entity Framework Core generate --schema=database/Entity Framework Core/schema.Entity Framework Core
pm2 reload mc-api
```

**`Entity Framework Core db push` fails with auth error**
Confirm the DB creds in `.deploy/.env.deploy` match the DreamHost-managed MSSQL. The script URL-encodes `@` in the password automatically; if your password has other special chars (`/`, `:`, `?`, `#`, `&`), they need to be safe for a URL â€” change them, or override `DATABASE_URL` directly in `.deploy/.env.deploy`.
...
Use the bundled recovery script â€” it lands in `<API_REMOTE_PATH>/scripts/` on every deploy:

```powershell
# From your laptop (PowerShell)
ssh madprospects@<host> "cd /home/madcreate-api && node scripts/reset-super-admin.js admin@madcreate.local <newpassword>"
```
